...
public class BlockManager {
	...
	private static Semaphore s1 = new Semaphore(-9); // TASK 4

	private static Semaphore s2 = new Semaphore(1);  //TASK 5

	public static int counter = 0;

	public static int counter2 = 0;  //TASK 5
	...
	public static void main(String[] argv) {
		try {
			...
			System.out
					.println("main(): Three AcquireBlock threads have been created.");
			...
			System.out
					.println("main(): Three ReleaseBlock threads have been created.");
			...
			System.out
					.println("main(): CharStackProber threads have been created: "
							+ NUM_PROBERS);
			...
			System.out.println("Final value of stack top-1 = "
					+ soStack.getAt(soStack.getITop() - 1) + ".");
			System.out.println("Stack access count = "
					+ soStack.getAccessCounter());

			System.exit(0);
		} catch (InterruptedException e) {
			System.err.println("Caught InterruptedException (internal error): "
					+ e.getMessage());
	...
	static class AcquireBlock extends BaseThread {
		...
		public void run() {
			System.out.println("\nAcquireBlock thread [TID=" + this.iTID
					+ "] starts executing.");
			...
			try {
				...
				System.out.println("AcquireBlock thread [TID=" + this.iTID
						+ "] has obtained Ms block " + this.cCopy
						+ " from position " + (soStack.getITop() + 1) + ".");

				System.out.println("Acq[TID=" + this.iTID
						+ "]: Current value of top = " + soStack.getITop()
						+ ".");

				System.out.println("Acq[TID=" + this.iTID
						+ "]: Current value of stack top = " + soStack.pick()
						+ ".\n");
			...
			while (!turnTestAndSet()) {
				...
				System.out
						.println("\n***Thread "
								+ this.iTID
								+ " has attempted to access PHASE2, but is waiting***\n");
				s2.P();
			}
			phase2();
			s2.V();
	...
	static class ReleaseBlock extends BaseThread {
		...
		public void run() {
			System.out.println("\nReleaseBlock thread [TID=" + this.iTID
					+ "] starts executing.");
			...
			try {
				...
				System.out.println("\nReleaseBlock thread [TID=" + this.iTID
						+ "] returns Ms block " + this.cBlock + " to position "
						+ (soStack.getITop() + 1) + ".");

				soStack.push(this.cBlock);

				System.out.println("Rel[TID=" + this.iTID
						+ "]: Current value of top = " + soStack.getITop()
						+ ".");
			...
			s2.V();
			mutex.P();
			System.out.println("ReleaseBlock thread [TID=" + this.iTID
					+ "] terminates.\n");
			if (this.iTID == 10) {
				System.out
						.println("\n****ALL THREADS HAVE COMPLETED PHASE 2****\n");
			...
			try {
				for (int i = 0; i < siThreadSteps; i++) {
					...
					for (int s = 0; s < soStack.getISize(); s++)
						System.out
								.print((s == BlockManager.soStack.getITop() ? "("
										: "[")
